#!/usr/bin/env python3
"""
E2E Test: USB CDC-ACM Hello + Echo over USBIP -- STM32F411 BlackPill
...
Architecture:
    ...
    USBCDCPeripheral (USB_OTG_FS @ 0x50000000, IRQ 67)
      | TCP proxy protocol [single event loop]
    QEMU slab-cortex-m (Cortex-M4) + F411_CDC_Blinky.bin
      | STM32F411PeripheralSet (RCC, PWR, GPIO, USART, etc.)

Run:
    PYTHONPATH=slab/python python3 slab/examples/cortex-m/stm32/f411/demos/cdc_blinky/test_e2e_usb_cdc_echo.py
...
from slab_stm32 import STM32F411PeripheralSet
...
class F411CDCEchoServer:
    """Async TCP server for QEMU + USBIP, running in one event loop.

    Dispatches register accesses to STM32F411PeripheralSet (which includes
    USBCDCPeripheral at 0x50000000). USBIP server and CDCBridge run in the
    ...
    def __init__(self, periph_port: int = PERIPH_PORT, usbip_port: int = 3240):
        ...
        # Create peripheral set (includes USB OTG FS)
        self.stm32 = STM32F411PeripheralSet()
...
def run_e2e_test(verbose: bool = False):
    ...
    print()
    print("=" * 70)
    print("  STM32F411 BlackPill USB CDC-ACM Hello + Echo over USBIP - E2E Test")
    print("=" * 70)
    ...
    firmware = os.path.join(test_dir, 'build', 'F411_CDC_Blinky.bin')
    ...
    e2e = F411CDCEchoServer(periph_port=periph_port, usbip_port=usbip_port)
    e2e.start_threaded()
    ...
    # -------------------------------------------------------------------------
    print()
    print("  [2/6] Starting QEMU with F411 CDC Blinky firmware...")
    machine_props = (
        f"slab-cortex-m"
        f",cpu-type=cortex-m4"
        f",tcp-port={periph_port}"
        f",sysclk-hz=100000000"
    )
    ...
    print(f"         Firmware: F411_CDC_Blinky.bin")
...
def _cleanup(qemu_proc, e2e, sock, results):
    ...
    if all_pass:
        ...
        print("  Full stack: USBIP <-> CDCBridge <-> OTG_FS <-> QEMU <-> F411 Firmware")
    else:
